...
export NODE_SALT=$(aws ssm get-parameters --region ap-northeast-2 --names NODE_SALT --query Parameters[0].Value | sed 's/"//g')
export DATABASE_SERVER_PORT=$(aws ssm get-parameters --region ap-northeast-2 --names DATABASE_SERVER_PORT --query Parameters[0].Value | sed 's/"//g')
